...
aggpreds <- ggplot(df_fed, aes(y = lpdens, x = t, col = method)) +
    geom_line() +
    theme_classic() + 
    labs(
        title = "Predictive ability of caliper v baseline versions",
        subtitle = "FEDFUNDS. Dynamic caliper width.",
        x = "Time",
...
ggsave("temp/final_dynamic_fedfunds.pdf", final)

# How well the methods do
df_fed <- data.table(df_fed)
df_fed[, .(mean_predab = mean(lpdens)), .(method)]